raw_str = '''kAewtloYgcFQaJNhHVGxXDiQmzjfcpYbzxlWrVcqsmUbCunkfxZWDZjUZMiGqhRRiUvGmYmvnJIHEmbT
...
lst = []
for index, symbol in enumerate(raw_str):
    if raw_str[index].islower() and raw_str[index-1].isupper() \
            and raw_str[index-2].isupper() and raw_str[index-3].isupper() \
            and raw_str[index+1].isupper() and raw_str[index+2].isupper() and raw_str[index+3].isupper() \
            and raw_str[index-4].islower() and raw_str[index+4].islower():
        lst.append(symbol)
    else:
        continue
print("".join(lst))
